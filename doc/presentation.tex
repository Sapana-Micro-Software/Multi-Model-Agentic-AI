...
\begin{algorithm}[H]
\caption{Recursive Validation Algorithm}
\begin{algorithmic}
\REQUIRE Input string $s$, validator $v$, sanitizer $san$
\ENSURE Validated and sanitized string
...
\node[state, below of=closed] (half) {HALF\_OPEN};
\draw[->] (closed) edge node[above] {Failures $\geq$ threshold} (open);
\draw[->] (open) edge node[right] {Timeout} (half);
\draw[->] (half) edge node[left] {Success} (closed);
\draw[->] (half) edge node[below] {Failure} (open);
\end{tikzpicture}
...
\begin{itemize}
    ...
    \item Copyright (C) 2025, Shyamal Chandra
\end{itemize}
\end{frame}
